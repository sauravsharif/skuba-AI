...
const OutfitStack: React.FC<OutfitStackProps> = ({ outfitHistory, onRemoveLastGarment }) => {
  return (
    <div className="flex flex-col h-full">
      <h2 className="text-xl font-bold tracking-tight text-gray-800 border-b border-gray-400/50 pb-2 mb-3">Outfit Stack</h2>
      <div className="space-y-2 overflow-y-auto pr-1 flex-grow">
          {outfitHistory.map((layer, index) => (
            <div
              key={layer.garment?.id || 'base'}
              className="flex items-center justify-between bg-white/50 p-2 rounded-lg border border-gray-200/80 backdrop-blur-sm"
            >
              <div className="flex items-center overflow-hidden">
                  ...
                  {layer.garment && (
                      <img src={layer.garment.url} alt={layer.garment.name} className="flex-shrink-0 w-12 h-12 object-cover rounded-md mr-3 bg-white" />
                  )}
