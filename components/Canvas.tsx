    ...
    <div className="w-full h-full flex items-center justify-center p-4 relative animate-zoom-in group">
      ...
      <div className="relative w-full h-full flex items-center justify-center">
        <AnimatePresence mode="wait">
          {displayImageUrl ? (
            ...
            <motion.div 
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="w-[300px] h-[450px] sm:w-[400px] sm:h-[600px] bg-gray-50 border border-gray-200 rounded-lg flex flex-col items-center justify-center text-center p-6"
            >
                <Spinner />
                <p className="text-md font-medium text-gray-600 mt-4">Loading Model...</p>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <AnimatePresence>
          {isLoading && (
              <motion.div
                  className="absolute inset-0 bg-white/60 backdrop-blur-sm flex flex-col items-center justify-center z-20 rounded-lg"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
              >
                  <Spinner />
                  {loadingMessage && (
                      <p className="text-lg font-medium text-gray-800 mt-4 text-center px-4">{loadingMessage}</p>
                  )}
              </motion.div>
          )}
        </AnimatePresence>
